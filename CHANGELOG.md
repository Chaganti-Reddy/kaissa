...
- Expeditions: master an opening by playing it against a bot until you win a set number of games. Each starts you from the opening's position and keeps your record; ships with the Italian, Ruy Lopez, Sicilian, Queen's Gambit, and London.
- Accessibility: optional shape markers on highlighted squares - a ring for check, a corner bracket for the last move - so the board reads without relying on colour. Off by default; a toggle in Settings.
- More content: Studies adds the Sicilian Najdorf, French Winawer, and Caro-Kann (six chapters now); Guess the Move adds Reti-Tartakower (Vienna 1910), an eleven-move queen-sacrifice mate.
...
- Home: rebuilt into a dashboard - a rating / tier / day-streak / due snapshot and cards that launch every mode (with a solved-today marker on the daily puzzle), now sharing the same nav rail as every other page.
- Calibrate: rebuilt into a proper onboarding flow - an intro screen, a short adaptive run with a progress bar, side-to-move badge and green/red feedback per answer, and a rating reveal with a level descriptor. Works on the 2D or 3D board.
- Settings: rebuilt with a live board preview, clickable board-theme swatches, and grouped toggle rows (board and pieces, gameplay, sound and display, data). Changing the theme or coordinates updates the preview instantly, and resetting progress now asks for confirmation.
- Piece art: the flat 2D board now draws real piece art instead of text symbols, with a choice of 14 sets (Cburnett, Merida, Chessnut, Fantasy, Spatial, Celtic, Pixel, Rhosgfx, Pirouetti, Shapes, Letter, Kiwen-Suwi, MPChess, Mono), selectable in Settings. The sets are rasterized from the open-source SVG sets distributed with Lichess; only sets under GPLv3-compatible licenses are included, credited in THIRD-PARTY-NOTICES.md.
- Sound: real sampled board sounds replace the synthesized cues, with four selectable sound themes (Sfx, Piano, NES, Futuristic) plus the original Classic procedural set, chosen in Settings. Any event a theme does not provide falls back to a built-in cue, so the game is never silent. The sounds are the open-source (AGPLv3) sets distributed with Lichess; only license-clean sets are included.
- Motion: the flat board gained chess.com-style feel - captured pieces shrink and fade out, moves glide with an ease-out instead of a linear slide, a grabbed piece lifts slightly, and solving a puzzle plays a celebratory flourish over the board.
- Page transitions: moving between screens now fades out and back in instead of cutting, so navigation feels continuous.
- Startup animation: launching the app shows a brief branded intro (the Kaissa mark with a loading bar) that fades into the home screen while the engine and content warm up in the background.
- Win cues: winning a game (by checkmate, resignation, or on time), passing an endgame drill, and setting a Puzzle Blitz best now play a victory cue, and board wins show the same celebratory flourish as a solved puzzle; losses and draws get their own sounds. Giving check briefly pulses the checked king's square.
